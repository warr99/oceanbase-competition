...
def __gen_perf(cluster_home_path:str) -> None:
    observer_bin_path = __observer_bin_path(cluster_home_path)
    pid = subprocess.getoutput(f'pidof {observer_bin_path}')
    if pid:
        _logger.info('start to gen perf.data. pid=%d, sleep=48')
        # 定义一个新的函数来运行 subprocess.run
        def run_perf_command():
            subprocess.run(f'perf record -F 99 -p {pid} -g -o /home/ChenXr/FlameGraph/perf.data -- sleep 48', shell=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--perf', action='store_true', help='monitor the entire process and generate perf.data')
