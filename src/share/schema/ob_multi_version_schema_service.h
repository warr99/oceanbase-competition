...
public:
  ...
  int refresh_and_add_schema(const common::ObIArray<uint64_t> &tenant_ids,
                             bool check_bootstrap = false, ObSArray<ObTableSchema> *bootstrap_schemas = NULL);
...
private:
  ...
  int refresh_tenant_schema(const uint64_t tenant_id, ObSArray<ObTableSchema> *schema_bootstrap = NULL);
